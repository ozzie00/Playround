...
import com.oneme.toplay.track.content.DescriptionGeneratorImpl;
import com.oneme.toplay.track.content.TracksLocation;
import com.oneme.toplay.track.content.TracksProvider;
import com.oneme.toplay.track.content.TracksProviderUtils;
import com.oneme.toplay.track.content.TracksProviderUtils.LocationIterator;
...
import com.oneme.toplay.track.services.tasks.AnnouncementPeriodicTaskFactory;
import com.oneme.toplay.track.services.tasks.PeriodicTaskExecutor;
import com.oneme.toplay.track.services.tasks.SplitPeriodicTaskFactory;
...
import com.oneme.toplay.track.util.SystemUtils;
import com.oneme.toplay.track.util.TrackIconUtils;
import com.oneme.toplay.track.util.TrackNameUtils;
...
public class TrackRecordingService extends Service {
  ...
  public static final String
      RESUME_TRACK_EXTRA_NAME = "com.oneme.toplay.track.RESUME_TRACK";
  ...
  protected void startForegroundService(PendingIntent pendingIntent, int messageId) {
    ...
    //        R.layout.ome_track_notif_expanded);
    //remoteViewsExpanded.setOnClickPendingIntent(R.id.swn_expanded_hitspace, pendingIntent);
    //remoteViewsExpanded.setChronometer(
    //        R.id.swn_expanded_chronometer, R.string.my_tracks_app_name, null, false);
    //remoteViewsExpanded.
    //        setImageViewResource(R.id.notification_icon, R.drawable.ic_stat_notify_recording);

    //Notification builder = new Notification.Builder(getApplicationContext()).setContentIntent(
    ...
    //        .setLocalOnly(true)
    //        .build();
    //builder.bigContentView = remoteViewsExpanded;
  ...
  private void restartTrack(Track track) {
    ...
    try {
      locationIterator = myTracksProviderUtils.getTrackPointLocationIterator(
          track.getId(), -1L, false, TracksProviderUtils.DEFAULT_LOCATION_FACTORY);
  ...
  private void onLocationChangedAsync(Location location) {
    try {
      ...
      if (currentRecordingInterval != locationListenerPolicy.getDesiredPollingInterval()) {
        ...
        location = new TracksLocation(location);
  ...
  private void showNotification(boolean isGpsStarted) {
    if (isRecording()) {
      if (isPaused()) {
        ...
        Intent intent = IntentUtils.newIntent(this, TrackDetailNextActivity.class)
            .putExtra(TrackDetailNextActivity.EXTRA_TRACK_ID, recordingTrackId);
        PendingIntent pendingIntent = TaskStackBuilder.create(this)
            .addParentStack(TrackDetailNextActivity.class).addNextIntent(intent)
