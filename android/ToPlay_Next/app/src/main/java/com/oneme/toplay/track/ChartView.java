...
import com.oneme.toplay.track.stats.ExtremityMonitor;
...
public class ChartView extends View {
  ...
  public static final int Y_AXIS_INTERVALS = 10;
  ...
  private static final int TARGET_X_AXIS_INTERVALS = 10;
  ...
  public ChartView(Context context) {
    ...
    gridPaint.setColor(context.getResources().getColor(android.R.color.white));
  ...
  /**
   * Draws the y axis(original version).
   * 
   * @param canvas the canvas
   */
  private void drawOrigYAxis(Canvas canvas) {
    int x = getScrollX() + leftBorder;
    int y = topBorder;
  ...
  private void drawYAxis(Canvas canvas) {
    ...
    //for (int i = 0; i < series.length; i++) {
      int index = series.length - 1;// - i;
      ChartValueSeries chartValueSeries = series[ELEVATION_SERIES];
      if (chartValueSeries.isEnabled() && chartValueSeries.hasData() || allowIfEmpty(index)) {
        markerXPosition -= drawYAxisMarkers(chartValueSeries, canvas, markerXPosition) + spacer;
      }
    //}
  ...
  /**
   * Draws the y axis for speed.
   *
   * @param canvas the canvas
   */
  private void drawSpeedYAxis(Canvas canvas) {
    int x = width - rightBorder;//getScrollX() + width;
    int y = topBorder;
    canvas.drawLine(x, y, x, y + effectiveHeight, axisPaint);

    int markerXPosition = x +  5*spacer;
   // for (int i = 0; i < series.length; i++) {
      int index = series.length - 1;// - i;
      ChartValueSeries chartValueSeries = series[SPEED_SERIES];
      if (chartValueSeries.isEnabled() && chartValueSeries.hasData() || allowIfEmpty(index)) {
        markerXPosition -= drawYAxisMarkers(chartValueSeries, canvas, markerXPosition) + spacer;
      }
    //}
