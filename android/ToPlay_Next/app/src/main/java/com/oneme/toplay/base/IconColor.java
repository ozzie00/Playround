...
public class IconColor {
    ...
    public static String iconColorAsString(Boolean isOnline, ToxUserStatus status)
    {
        String color;
        if (!isOnline) {
            color = ("#" + AppConstant.OMETOPLAYGRAY); //offline
        } else if (status == ToxUserStatus.TOX_USERSTATUS_NONE) {
            color = ("#" + AppConstant.OMETOPLAYOLYMPICGREEN); //online
        } else if (status == ToxUserStatus.TOX_USERSTATUS_AWAY) {
            color = "#" + AppConstant.OMETOPLAYOLYMPICYELLOW; //away
        } else if (status == ToxUserStatus.TOX_USERSTATUS_BUSY) {
            color = "#" + AppConstant.OMETOPLAYOLYMPICRED; //busy
        } else {
            color = "#" + AppConstant.OMETOPLAYOLYMPICBLUE;
        }
        return color;
    }
