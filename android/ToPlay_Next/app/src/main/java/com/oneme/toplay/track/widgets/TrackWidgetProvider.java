...
public class TrackWidgetProvider extends AppWidgetProvider {
  ...
  private static RemoteViews getRemoteViews(Context context, long trackId, int heightSize) {
    ...
    TracksProviderUtils myTracksProviderUtils = TracksProviderUtils.Factory.get(context);
    if (trackId == -1L) {
  ...
  private static void updateStatisticsContainer(
    ...
    if (track != null) {
      Intent intent = IntentUtils.newIntent(context, TrackDetailNextActivity.class)
          .putExtra(TrackDetailNextActivity.EXTRA_TRACK_ID, track.getId());
      pendingIntent = TaskStackBuilder.create(context)
          .addParentStack(TrackDetailNextActivity.class).addNextIntent(intent).getPendingIntent(0, 0);
    } else {
  ...
  private static void updateRecordStatus(
    ...
    if (isRecording) {
      ...
      colorId = recordingTrackPaused ? R.color.red : R.color.playround_default;
    } else {
      status = "";
